...
total_qtr_final = total_qtr_long.pivot_table(index=['state', 'drugtype', 'temporal_unit', 'date', 'metric'], columns = 'type', values = 'value')
...
generic_brand_qtr_final = generic_brand_qtr_long.pivot_table(index=['state', 'drugtype', 'temporal_unit', 'date', 'metric'], columns = 'genericind', values = 'value')
...
total_yr_final = total_yr_long.pivot_table(index=['state', 'drugtype', 'temporal_unit', 'date', 'metric'], columns = 'type', values = 'value')
...
generic_brand_yr_long = pd.melt(generic_brand_yr, id_vars=['state', 'drugtype', 'temporal_unit', 'date', 'genericind'],
                                value_vars = ['rx', 'adjmedamt'], var_name = 'metric')
generic_brand_yr_final = generic_brand_yr_long.pivot_table(index=['state', 'drugtype', 'temporal_unit', 'date', 'metric'], columns = 'genericind', values = 'value')
...
final.replace({'drug': 'bup'}, 'buprenorphine', inplace = True)
# probably also want to adjust metric names but wait on final labels before doing this

#quarterly_final.to_csv('opioids_data.csv', index = False)